...
def test_get_auth_metadata_plugin_oauth_should_pass(post, config):
    auth_metadata_plugin = get_auth_metadata_plugin(config)
...
def test_get_auth_metadata_plugin_oauth_should_raise_when_response_is_not_200(post, config):
    with raises(RuntimeError):
        get_auth_metadata_plugin(config)
        assert post.call_count == 1
        assert post.call_args == call(AUTH_URL, headers=HEADERS, data=DATA)
